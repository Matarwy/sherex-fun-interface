        ...
        const checkSendTx = async (): Promise<void> => {
          ...
          const cbk = (signatureResult: SignatureResult) => {
            ...
            handleMultiTxToast({
              ...
              meta: {
                ...swapMeta,
                title: isSlippageError ? 'Swap failed due to slippage error!' : swapMeta.title,
                description: isSlippageError ? 'Slippage has exceeded user settings.\nTry again or adjust your slippage tolerance.' : swapMeta.description
              },
              isSwap: true,
              handler,
              getSubTxTitle,
    ...
    unWrapSolAct: async ({ amount, onSent, onError, ...txProps }): Promise<string | undefined> => {
      ...
      if (builder.allInstructions.length > 12) {
        ...
        const meta = {
          ...
          description: 'Unwrapped WSOL to SOL',
          txHistoryTitle: 'Unwrapped all WSOL',
          txHistoryDesc: 'Unwrapped WSOL to SOL',
          txValues: {}
        }

        const getSubTxTitle = () => 'Unwrapped WSOL to SOL'
        multiExecute({
          sequentially: true,
          onTxUpdate: (data) => {
      ...
      const meta = {
        title: `Unwrapped all WSOL`,
        description: `Unwrapped total ${values} WSOL`,
        txHistoryTitle: 'Unwrapped all WSOL',
        txHistoryDesc: `Unwrapped total ${values} WSOL`,
        txValues: values
      }
