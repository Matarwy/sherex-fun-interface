...
export function SwapPanel({
  ...
  const [outputMint, setOutputMint] = useState<string>(SHEREX.address)
  ...
  const balanceNotEnough = balanceAmount.lt(inputAmount || 0) ? 'Insufficent balance' : undefined
  ...
  let swapError = balanceNotEnough

  switch (error) {
    case "ROUTE_NOT_FOUND":
      swapError = "No Routes Found"
      break
    case "AMOUNT_TOO_SMALL":
      swapError = "Swap Amount Too Small"
      break
    case "TOO_LOW_LIQUIDITY":
      swapError = "Insufficient Liquidity"
      break
    case "INSUFFICIENT_LIQUIDITY":
      swapError = "Insufficient Liquidity"
      break
    case "REQ_POOL_NOT_OPEN":
      swapError = "Pool Open At"
      break
    default:
      break
  }
    ...
    <>
      ...
      {isSolFeeNotEnough ? (
        ...
        >
          ...
          <Text>{`You need at least ${formatToRawLocaleStr(DEFAULT_SOL_RESERVER)} SOL to pay for fees and deposits`}</Text>
        </Flex>
      ) : null}
          ...
          {inputFeeConfig && tokenInput ? (
            ...
            >
              <Box
                ...
                borderRadius="4px"
              >
                {getMintSymbol({ mint: tokenInput })} ({inputFeeConfig[feeKeys.input].transferFeeBasisPoints / 100}% Tax)
              </Box>
            </Tooltip>
          ) : null}
          ...
          {outputFeeConfig && tokenOutput ? (
            ...
            >
              <Box
                ...
                borderRadius="4px"
              >
                {getMintSymbol({ mint: tokenOutput })} ({outputFeeConfig[feeKeys.output].transferFeeBasisPoints / 100}% Tax)
              </Box>
            </Tooltip>
          ) : null}
        </Flex>
      ...
      <ConnectedButton
        ...
        loadingText={<div>{isSending ? "Transaction initiating" : isComputing ? "Computing.." : ''}</div>}
        onClick={isHighRiskTx ? onHightRiskOpen : handleClickSwap}
      >
        <Text>
          {swapDisabled ? 'Disabled' : swapError || 'Swap'}
...
function TransferFeeTip({ feeConfig, token }: { feeConfig: TransferFeeDataBaseType; token: TokenInfo }) {
  ...
  return (
    <>
      ...
      <Text color={colors.primary}>This token uses the Token2022 Program, which provides a set of token extensions to be enabled by the token creator.</Text>
