      ...
      >
        ...
        <HStack gap={4} py={1} justifyContent="space-between">
          <ItemLabel
            ...
            tooltip={isBaseOut ? 'The maximum number of tokens you will input on this trade' : 'The minimum number of tokens you will receive. This is determined by your slippage tolerance.'}
          />
          <MinimumReceiveValue tokenOutput={isBaseOut ? tokenInput : tokenOutput} amount={computedSwapResult?.otherAmountThreshold || ''} />
        </HStack>
        <HStack gap={4} py={1} justifyContent="space-between">
          <ItemLabel name='Price Impact' tooltip="The difference between the current market price and estimated price due to trade size" />
          <Text
            fontSize="xs"
        ...
        <Collapse in={showMoreSwapInfo} animateOpacity>
          <HStack gap={4} py={1} justifyContent="space-between">
            <ItemLabel name="Order Routing" tooltip="This route gave the best price for your trade" />
            {routeTokens && <RoutingValue routePlan={computedSwapResult?.routePlan || []} />}
          </HStack>

          <HStack gap={4} py={1} justifyContent="space-between">
            <ItemLabel name="Estimated Fees" tooltip="Swap fees go to LPs, RAY buybacks, and treasury." />
...
function PriceDetector({
  ...
  return (
    <HStack>
      ...
      {priceImpact && <Tooltip label={priceImpact === 'low' ? 'Low Price Impact' : 'Price Impact Warning'}>
        {priceImpact === 'low' ? (
          <CircleCheckBreaker />
        ) : priceImpact === 'warning' ? (
          <WarningIcon />
