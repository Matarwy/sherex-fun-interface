...
const SherexTokenListCard = ({
  tokens,
  isLoading = false,
  onLoadMore,
  hasMore = false
}: {
  tokens: SherexMintInfo[]
  isLoading?: boolean
  onLoadMore?: () => void
  hasMore?: boolean
}) => {
    ...
    <listContext.Provider value={contextValue}>
      ...
      >
        {isLoading
          ...
          : tokens.map((token) => (
              <ListItem key={token.mintId}>
                <motion.div
                  layoutId={token.mintId}
                  layout="position"
                  transition={{
                    type: 'spring',
                    bounce: 0.1,
...
const TokenCard = ({ token }: { token: SherexMintInfo }) => {
  const { colorMode } = useColorMode()
  const isLight = colorMode === 'light'
  const router = useRouter()
  const queryReferrer = useReferrerQuery('&')
  const { onCopy: copy, hasCopied } = useClipboard(token.mintId)

  return (
    <Box
      background={
        Number(token.finishingRate) >= HEATING_RATE
          ? isLight
      ...
      cursor="pointer"
      onClick={() => {
        router.push(`/token?mint=${token.mintId}${queryReferrer}`)
      }}
      _before={
        Number(token.finishingRate) >= HEATING_RATE
          ? isLight
          ...
          <Flex color={colors.textLaunchpadLink} alignItems="center" gap="1">
            {/* {token.defaultCurve ? null : (
              <Tooltip
                hasArrow
                placement="top"
                label={'This token has custom Sherex parameters set by the creator. Make sure to check the token page for full details.'}
              >
                <Box>
                  <Curve />
                </Box>
            ...
            <Flex justifyContent="space-between">
              <Grid templateRows="1fr auto" alignItems="center" rowGap={1}>
                <Flex gap={2} alignItems="center" lineHeight="18px">
                  <Text
                    ...
                    sx={
                      Number(token.finishingRate) >= HEATING_RATE
                        ? {
                            background: isLight ? '#aaef6e' : 'linear-gradient(245.22deg, #FF9C32 7.97%, #FFD000 39.94%, #EDFFDC 92.1%)',
                            WebkitBackgroundClip: 'text',
                            WebkitTextFillColor: 'transparent',
                            backgroundClip: 'text'
                          }
                ...
                <Flex justifyContent="space-between" alignItems="center">
                  <ThreeStageProgress
                    percent={Number(token.finishingRate)}
                    sx={{
                      width: '10rem'
                    }}
                  />
...
export default SherexTokenListCard
