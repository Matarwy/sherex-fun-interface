...
export default function TradeBox({
  ...
  const swapError = error ? (
    error === "ROUTE_NOT_FOUND" ? 'No Routes Found' : 
    error === "AMOUNT_TOO_SMALL" ? "Swap Amount Too Small" : 
    error === "TOO_LOW_LIQUIDITY" ? "Insufficient Liquidity" : 
    error === "INSUFFICIENT_LIQUIDITY" ? "Insufficient Liquidity" :
    error === "REQ_POOL_NOT_OPEN" ? "Pool Open At" :
    ""
  ) : balanceNotEnough
