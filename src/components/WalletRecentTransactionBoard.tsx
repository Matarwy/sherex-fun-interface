      ...
      <Box>
        <HStack fontWeight={500} my={3} fontSize="sm" justify="space-between" color={colors.textSecondary}>
          ...
          <HStack spacing={0.5} cursor="pointer" onClick={turnOn}>
            <Text>View all</Text> <ChevronRightIcon width={'14px'} height={'14px'} />
          </HStack>
        </HStack>
  ...
  return (
    <Drawer variant="flatScreenEdgePanel" size="sm" isOpen={isOpen} onClose={onClose} trapFocus={false}>
      ...
      <DrawerContent>
        <DrawerCloseButton />
        {/* hidden transaction */}
        <DrawerHeader display="none">Recent transactions</DrawerHeader>
                  ...
                  >
                    ...
                    <HStack gridArea={'name'} fontSize="xs" color={colors.textSecondary} gap={1}>
                      <Box
                        ...
                        whiteSpace="nowrap"
                      >
                        {subTransaction.name || `Transaction ${idx + 1}`}
                      </Box>
                      {subTransaction.txId && (
                        <ExternalLinkLargeIcon
                          cursor="pointer"
