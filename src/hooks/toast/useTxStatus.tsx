      ...
      .subscribe(
        ({
            ...
            if (signatureResult.err) {
              ...
              toastSubject.next({
                ...
                title: isSlippageError
                  ? 'Swap failed due to slippage error!'
                  : (isMultisigWallet ? (
                      <>
                ...
                status: 'error',
                description: isSlippageError ? 'Slippage has exceeded user settings.\nTry again or adjust your slippage tolerance.' : description || `${explorerUrl}/tx/${txId}`,
          ...
          timeout = window.setTimeout(() => {
            ...
            toastSubject.next({
              title: 'Send transaction timeout',
              description,
              status: 'warning',
              duration: 8 * 1000,
              onClose
          ...
          const renderDetail = () => {
            return (
              <Flex flexDirection="column" gap="3">
                {subTxIds.map(({ txId, title = 'Transaction' }, idx) => (
                  <Box
              ...
              const cbk = (signatureResult: SignatureResult, context: Context) => {
                ...
                if (signatureResult.err) {
                  ...
                  toastSubject.next({
                    ...
                    status: 'error',
                    description: isSlippageError ? 'Slippage has exceeded user settings.\nTry again or adjust your slippage tolerance.' : description,
                    detail: renderDetail(),
                    onClose
                  })
